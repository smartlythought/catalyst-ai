...
function buildDailyPicksHTML(picks: DailyPick[], date: string): string {
  const buyPicks = picks.filter(p => p.action === "BUY");
  const sellPicks = picks.filter(p => p.action === "SELL");

  const renderPick = (p: DailyPick) => {
    const isBuy = p.action === "BUY";
    const color = isBuy ? "#16C784" : "#EA3943";
    const emoji = isBuy ? "🟢" : "🔴";
    const price = p.currentPrice || p.entryPrice;
    const upside = isBuy
      ? ((p.targetPrice - p.entryPrice) / p.entryPrice * 100).toFixed(1)
      : ((p.entryPrice - p.targetPrice) / p.entryPrice * 100).toFixed(1);
    ...
    return `
      <div style="background:#111827;border:1px solid #1F2937;border-radius:12px;padding:16px;margin-bottom:8px">
        <div style="display:flex;align-items:center;margin-bottom:8px">
          ...
          <span style="font-size:18px;font-weight:800;color:white;font-family:monospace">${p.symbol}</span>
          <span style="margin-left:8px;padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;font-family:monospace;color:${color};background:${color}1A;letter-spacing:0.5px">${p.action}</span>
          <span style="margin-left:auto;font-family:monospace;font-size:14px;font-weight:700;color:white">$${price.toFixed(2)}</span>
        </div>
        <div style="font-size:12px;color:#9CA3AF;margin-bottom:6px">${p.companyName}</div>
        <table style="width:100%;border-collapse:collapse;font-size:12px;color:#9CA3AF">
          <tr>
            <td style="padding:3px 0">Entry <span style="float:right;color:white;font-family:monospace;font-weight:600">$${p.entryPrice.toFixed(2)}</span></td>
            <td style="padding:3px 0;padding-left:16px">Target <span style="float:right;color:${color};font-family:monospace;font-weight:600">$${p.targetPrice.toFixed(2)} (${upside}%)</span></td>
          </tr>
          <tr>
            <td style="padding:3px 0">Stop <span style="float:right;color:#EA3943;font-family:monospace;font-weight:600">$${p.stopLoss.toFixed(2)}</span></td>
            <td style="padding:3px 0;padding-left:16px">Conviction <span style="float:right;color:${color};font-family:monospace;font-weight:600">${p.conviction}%</span></td>
          </tr>
        </table>
        <p style="color:#D1D5DB;font-size:12px;line-height:1.5;margin:8px 0 0">${p.rationale}</p>
      </div>`;
  };
...
<body style="margin:0;padding:0;background:#0A0E14;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif">
  <div style="max-width:520px;margin:0 auto;padding:24px 16px">
    <div style="text-align:center;margin-bottom:20px">
      <span style="font-size:22px;font-weight:800;color:#E8743B;letter-spacing:-0.5px">Catalyst</span>
      <div style="font-size:11px;color:#6B7280;font-family:monospace;margin-top:4px">DAILY TOP 10 PICKS · ${date}</div>
    </div>

    <div style="background:#111827;border:1px solid #1F2937;border-radius:14px;padding:16px;margin-bottom:16px;text-align:center">
      <div style="font-size:32px;font-weight:800;color:white">${picks.length}</div>
      <div style="font-size:12px;color:#6B7280;font-family:monospace;margin-top:2px">
        <span style="color:#16C784">${buyPicks.length} BUY</span> · <span style="color:#EA3943">${sellPicks.length} SELL</span>
      </div>
    </div>

    ${buyPicks.length > 0 ? `
    <div style="font-size:10px;font-weight:700;color:#16C784;font-family:monospace;letter-spacing:1px;margin-bottom:8px;padding-left:4px">BUY PICKS</div>
    ${buyPicks.map(renderPick).join("")}` : ""}

    ${sellPicks.length > 0 ? `
    <div style="font-size:10px;font-weight:700;color:#EA3943;font-family:monospace;letter-spacing:1px;margin:16px 0 8px;padding-left:4px">SELL PICKS</div>
    ${sellPicks.map(renderPick).join("")}` : ""}

    <div style="text-align:center;padding:20px 0 8px">
      <a href="https://catalyst.claudeo.ai/picks" style="display:inline-block;padding:12px 32px;background:#E8743B;color:white;font-weight:700;font-size:14px;border-radius:10px;text-decoration:none">View All Picks →</a>
    </div>
    <p style="text-align:center;color:#4B5563;font-size:10px;margin-top:20px;font-family:monospace">
      Catalyst AI — Not financial advice. Past performance does not guarantee future results.<br>
      <a href="https://catalyst.claudeo.ai/account" style="color:#6B7280;text-decoration:underline">Manage alert preferences</a>
    </p>
  </div>
</body>
</html>`;
...
export async function sendDailyPicksDigest(picks: DailyPick[], tradingDate: string): Promise<number> {
  const resend = getResend();
  if (!resend) return 0;
  ...
  const html = buildDailyPicksHTML(picks, tradingDate);
  const buyCount = picks.filter(p => p.action === "BUY").length;
  const sellCount = picks.filter(p => p.action === "SELL").length;
  const topPick = picks[0];
  const subject = `📊 Daily Top ${picks.length}: ${buyCount} Buy, ${sellCount} Sell — ${topPick?.symbol || ""} ${topPick?.action || ""} @ $${topPick?.currentPrice?.toFixed(2) || topPick?.entryPrice?.toFixed(2) || ""}`;
